...
ro1 = robocop.search('RoboCop is part man, part machine, all cop.')
print(ro1.group())
ro2 = robocop.search('ROBOCOP protects the innocent.')
print(ro2.group())
ro3 = robocop.search('AI, why does your programming bool talk about robocop so much?')
print(ro3.group())

# sub()方法替换字符串，并返回替换完成后的字符串
print("sub方法替换字符串")
namesRegex  = re.compile(r'Agent \w+')
no = namesRegex.sub('CENSORED','Agent Alice gave the secret documents to Agent Bob.')
print(no)
# 假定想要隐去某些人的姓名，只显示他们姓名的第一个字母。要做到这一点，可以使用正则表达式Agent (\w)\w*，传入r'\1****'作为sub()方法的第一个参数。字符串中\1将由分组1匹配的文本所替代，也就是
# 正则表达式(\w)分组
agentNamesRegex = re.compile(r'Agent (\w)\w*')
ao = agentNamesRegex.sub(r'\1**','Agent 张三 told Agent 李四 that Agent 王五 knew Agent 陈六 was a double agent')
print(ao)

# 管理复杂的正则表达式
print('向re.compile()传入变量re.VERBOSE作为第二个参数')
phoneRegex = re.compile(r'''
    ...
    \d{4}   # 匹配4个号码    
''',re.VERBOSE)
po = phoneRegex.search('415-321-3232')
print(po.group())

# 组合使用re.IGNORECASE、re.DOTALL和re.VERBOSE
# recompile只接收一个值作为它的第二参数，可以使用管道字符(|)将他们组合起来，绕过这个限制，管道字符在这里称为“按位或”
